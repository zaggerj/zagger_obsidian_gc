...
# 18. rustdesk `198793Peng` `284 037 269`
...
# 20. patch windows上出错了 估计是换行符问题导致出错了 实测linux上创建文件，然后diff 之后patch是没有问题的
...
# 21. `grep '需求\|bug' ./src/components/gateway/*.vue`
...
# 29. windows git bash 中 查询端口占用 并干掉
...
# 30. 快速清空工作区 #clean #empty

`git checkout -- .;git clean -fd;`

# 31. mac干掉不要的文件 #find #\.DS_Store
...
# 43. 服务器代码 #上传 | [[汇总所有在线文档#^ce3eb8|scp]]
...
# 53. weboeidp 172.19.20.119 hzj 账号
...
# 54. weboeidp 172.16.201.136 getConnectInfo 补丁 nwy admin1
...
# 55. tspace 服务，服务器修改服务名称，避免其自动重启
...
# 61. vdi 4.4版本 模板连接失败 实际上用的vnc 6080端口
...
# 69. Flag Description
# 70. no 不自动重启容器：(默认value)
# 71. on-failure 容器发生error而退出(容器退出状态不为0)重启容器
# 72. unless-stopped 在容器已经stop掉或Docker stoped/restarted的时候才重启容器
# 73. always 在容器已经stop掉或Docker stoped/restarted的时候才重启容器

# 74. 如果已经启动的项目.则使用update更新
...
# 81. ssh-copy-id -i ~/.ssh/id_rsa.pub [zhangyao@172.16.103.196](mailto:zhangyao@172.16.103.196)
...
### 86.1.1. Vdi&[Voi@r00t](http://Voi@r00t)
...
# 88. [环境配置相关](siyuan://blocks/20230705103954-dsr1hc8 "环境配置相关-不常用")
