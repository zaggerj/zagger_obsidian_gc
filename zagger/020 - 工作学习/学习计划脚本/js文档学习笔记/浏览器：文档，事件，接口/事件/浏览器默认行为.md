...
Author:: zagger
Source URL::
Modify:: 2023-12-20 星期三 15:11:38
...
### 1.4.2. [阻止浏览器行为](https://zh.javascript.info/default-browser-action#zu-zhi-liu-lan-qi-hang-wei)

有两种方式来告诉浏览器我们不希望它执行默认行为：

- 主流的方式是使用 `event` 对象。有一个 `event.preventDefault()` 方法。
- 如果处理程序是使用 `on<event>`（而不是 `addEventListener`）分配的，那返回 `false` 也同样有效。（旧的方式）
...
### 1.4.3. [示例：菜单](https://zh.javascript.info/default-browser-action#shi-li-cai-dan)
...
```
![image.png](https://raw.githubusercontent.com/zaggerj/obsidian_picgo/main/obsidian/20231220151625.png)
...
所以我们在~~标记（markup）~~html中使用了 `<a>`。但通常我们打算处理 JavaScript 中的点击。因此，我们应该阻止浏览器默认行为。
...
![image.png](https://raw.githubusercontent.com/zaggerj/obsidian_picgo/main/obsidian/20231220155137.png)


这是因为浏览器行为在 `mousedown` 上被取消。如果我们用另一种方式进行输入，则仍然可以进行聚焦。例如，可以使用 Tab 键从第一个输入切换到第二个输入。但鼠标点击则不行。

### 1.4.4. [处理程序选项 “passive”](https://zh.javascript.info/default-browser-action#chu-li-cheng-xu-xuan-xiang-passive)
...
### 1.4.5. [event.defaultPrevented](https://zh.javascript.info/default-browser-action#eventdefaultprevented)
...
### 1.4.6. [总结](https://zh.javascript.info/default-browser-action#zong-jie)
